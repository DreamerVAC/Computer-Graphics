...
namespace TinyRenderer
{
	...
	glm::vec4 TRTexture2DSampler::textureSampling_bilinear(const TRTexture2D &texture, glm::vec2 uv)
	{
		...
		glm::vec4 pixel00 = glm::vec4(r, g, b, a);
		texture.readPixel(x0, y0 + 1, r, g, b, a);
		glm::vec4 pixel01 = glm::vec4(r, g, b, a);
		texture.readPixel(x0 + 1, y0, r, g, b, a);
		glm::vec4 pixel10 = glm::vec4(r, g, b, a);
		texture.readPixel(x0 + 1, y0 + 1, r, g, b, a);
		glm::vec4 pixel11 = glm::vec4(r, g, b, a);

		float x_ratio = x - x0;
		float y_ratio = y - y0;
		glm::vec4 top = pixel00 * (1 - x_ratio) + pixel10 * x_ratio;
		glm::vec4 bottom = pixel01 * (1 - x_ratio) + pixel11 * x_ratio;
